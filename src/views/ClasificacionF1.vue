...
// Cargar datos de Pilotos por defecto al montar el componente
onMounted(getDataPilotos);
</script>

<template>
  <main>
    <h2>Clasificación del Campeonato F1</h2>

    <!-- Botones para cambiar entre categorías -->
    <div class="button-group">
      <button @click="getDataPilotos" :disabled="isLoading">Clasificación F1 Pilotos</button>
      <button @click="getDataEquipos" :disabled="isLoading">Clasificación F1 Equipos</button>
    </div>
    ...
    <ul v-else-if="list.length > 0" class="table-list">
      <li v-for="(item, index) in list" :key="index" class="table-row">
        ...
        <div class="table-cell piloto">{{ item.nombre }}</div>
        <div class="table-cell puntos">
          {{ item.puntos }} pts 
          <span v-if="index !== 0" class="diferencia-puntos">(-{{ list[0].puntos - item.puntos }})</span>
        </div>
      </li>
    </ul>
...
<style scoped>
/* Estilo para la diferencia de puntos */
.diferencia-puntos {
  color: red;
  font-size: 10pt;
}
